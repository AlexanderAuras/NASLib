...
if sys.version_info.major != 3 or sys.version_info.minor < 7 or sys.version_info.minor > 14:
    raise ValueError(
...
with pathlib.Path(version_path).open() as fh:
...
try:
    ...
    if "--user" in sys.argv:
        subprocess.run([sys.executable, "-m", "pip", "install", "--upgrade", "--user", git_nasbench301], check=False)
    else:
        subprocess.run([sys.executable, "-m", "pip", "install", "--upgrade", git_nasbench301], check=False)
...
try:
    ...
    if "--user" in sys.argv:
        subprocess.run(
            [sys.executable, "-m", "pip", "install", "--upgrade", "--user", git_nasbench_pytorch],
            check=False,
        )
    else:
        subprocess.run([sys.executable, "-m", "pip", "install", "--upgrade", git_nasbench_pytorch], check=False)
